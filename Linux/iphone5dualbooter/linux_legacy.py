...
class LegacyLinuxError(RuntimeError):
    pass
...
def _request_bytes(url: str, timeout: int = 180) -> bytes:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "iPhone5DualBooter-Linux/1.0.0",
...
def _safe_extract(archive: zipfile.ZipFile, destination: Path) -> None:
    ...
    for member in archive.infolist():
        ...
        if target != destination and destination not in target.parents:
            raise LegacyLinuxError("Unsafe path found in the Legacy iOS Kit ZIP.")
...
def _latest_release_asset_url() -> str:
    ...
    if not zip_assets:
        raise LegacyLinuxError("The latest Legacy iOS Kit release has no ZIP asset.")
...
def ensure_legacy_kit(
    ...
    try:
        ...
        raise LegacyLinuxError(f"Could not download Legacy iOS Kit: {exc}") from exc
    ...
    try:
        ...
        raise LegacyLinuxError("The Legacy iOS Kit download was not a valid ZIP.") from exc
    finally:
        temp_path.unlink(missing_ok=True)
    root = find_legacy_root(download_root)
    if not root:
        raise LegacyLinuxError("The downloaded Legacy iOS Kit ZIP did not contain restore.sh.")
...
def run_legacy_native_restore(
    ...
    if os.name == "nt":
        raise LegacyLinuxError("This application requires Linux.")
    if not settings.stock_host_ipsw or not settings.stock_host_ipsw.is_file():
        raise LegacyLinuxError("Select the stock iOS 8.4.1 Restore IPSW.")
    ...
    while True:
        ...
        if action == "rerun":
            ...
            continue
        raise LegacyLinuxError("Legacy iOS Kit restore was cancelled.")
