...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("iPhone 5 DualBooter for Linux v1.0.3")
        self.resize(980, 860)
        self.app_root = Path(__file__).resolve().parents[1]
        self.settings_store = QSettings("iPhone5DualBooter", "Linux")
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("iPhone 5 DualBooter for Linux")
        title.setStyleSheet("font-size: 21px; font-weight: 700;")
        outer.addWidget(title)

        notice = QLabel(
            "Native Linux edition. Legacy iOS Kit runs directly on Linux using the standard native restore process."
        )
    ...
    def _start(self) -> None:
        ...
        if os.name == "nt":
            QMessageBox.critical(
                self,
                "Linux required",
                "This application requires Linux.",
            )
            return
        try:
    ...
    def _show_legacy_prompt(self, exit_code: int, attempt: int) -> None:
        ...
        box.setInformativeText(
            ...
            "Pick Run Legacy Again if it installed dependencies, updated itself, closed early, or showed an error. Legacy iOS Kit will handle pwnDFU again natively on Linux."
        )
